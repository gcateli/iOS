...
if (yesVotes1 == noVotes1){
    print("We've had a tie")
}
else if(yesVotes1 > noVotes1){
    print("Mascotts should change votes")
}
else{
    print("Mascotts should not change votes")
...
func printResults(issue: String,votes: [Bool]) -> String{
    var yes = 0
    var no = 0
    ...
    return "\(issue) \(yes) yes, \(no) no."
}

var result1  = printResults(issue: "Should we change the mascot?", votes: shouldMascotChangeVotes)
var result2  = printResults(issue: "Should we install Coffee Machines?", votes: shouldInstallCoffeeVendingMachineVotes)
var result3 =  printResults(issue: "Should we have more poll options?", votes: shouldHaveMorePollOptionsVotes)
